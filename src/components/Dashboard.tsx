...
import { AlertTriangle, CheckCircle, Map as MapIcon, Info, Settings, Radio, Loader2, Book, Menu } from 'lucide-react';
  ...
  const SidebarContent = () => (
    <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 mb-6 md:mb-8">
          <Card className="bg-red-500/10 border-red-500/20 p-0">
            <CardContent className="p-3">
              ...
              <span className="text-xl md:text-2xl font-bold text-red-400">{floodedCount}</span>
            </CardContent>
          </Card>
          <Card className="bg-orange-500/10 border-orange-500/20 p-0">
            <CardContent className="p-3">
              ...
              <span className="text-xl md:text-2xl font-bold text-orange-400">{riskCount}</span>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {devices.map((device) => (
                <Card
                  key={device._id}
                  className={cn(
                    "cursor-pointer transition-all hover:border-primary/50 hover:shadow-md min-h-[44px]",
      ...
      <div className="p-4 border-t border-border bg-card/50">
        <Button variant="secondary" className="w-full mb-2 py-2 min-h-[44px]">
          <Link to="/docs" className='flex items-center justify-center py-2 w-full' onClick={() => setMobileDrawerOpen(false)}>
        ...
        {isLoaded && user && (
          <Button variant="secondary" className="w-full mb-2 py-2 min-h-[44px]">
            <Link to="/admin" className='flex items-center justify-center py-2 w-full' onClick={() => setMobileDrawerOpen(false)}>
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-background text-foreground overflow-hidden">
      ...
      <Button
        variant="ghost"
        size="icon"
        className="md:hidden fixed top-4 left-4 z-1100 bg-background/90 backdrop-blur-sm shadow-lg min-h-[44px] min-w-[44px]"
      ...
      {mobileDrawerOpen && (
        <div
          className="md:hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-1050 transition-opacity"
          onClick={() => setMobileDrawerOpen(false)}
        />
      )}
      ...
      <div
        className={cn(
          "md:hidden fixed top-0 left-0 h-full w-80 max-w-[85vw] bg-card border-r border-border z-1100 flex flex-col transform transition-transform duration-300 ease-in-out overflow-hidden",
      ...
      {/* Desktop Sidebar */}
      <div className="hidden md:flex w-80 shrink-0 border-r border-border bg-card flex-col">
        <SidebarContent />
      </div>
